...
export const PROMPT_MAP = {
	...
	application: {
		authorization_error: {
			...
			status: "AUTH ERROR",
			code: "ADMIN",
			labels: ["Auth error!", "Auth error!"],
		},
		forbidden_exception: {
			id: 10000,
			status: "FORBIDDEN",
			code: "ADMIN",
			labels: ["Forbidden error!", "Forbidden error!"],
		},
		username_or_password_incorrect: {
			id: 10000,
			status: "BAD REQUEST",
			code: "ADMIN",
			labels: ["Username or password incorrect!", "Foydanuvchi nomi yoki parol noto'g'ri!"],
		},
		admin_not_found: {
			id: 10001,
			status: "NOT FOUND",
		...
		product_not_found: {
			...
			code: "PRODUCT",
			labels: [
				"Product not found by username!",
				"Product не найден по логину!",
				"Bunaqa id li product topilmadi!",
			],
		},
		user_not_found: {
			id: 10003,
			...
			labels: [
				...
				"Bunaqa telefon nomer li User topilmadi!",
			],
		},
		...
		product_update_not_allowed: {
			...
			code: "Product",
			labels: [
				"Product update not allowed!",
				"Обновление продукта не разрешено!",
				"Mahsulotni o'zgartirish mumkin emas!",
			],
		},
		product_doesnt_enough: {
			id: 10004,
			status: "BAD REQUEST",
			code: "Product",
			labels: [
				"Product doesn't enough!",
...
export function getPromptByCode<K1 extends keyof PromptMap>(origin: K1, code: string) {
	const flatPrompts = Object.entries(PromptsMap[origin]);
	for (const [key, value] of flatPrompts) {
		if ((value as Prompt).code === code) {
			return { key, value };
		}
	}
	return null;
...
export function getPrompt<K1 extends keyof PromptMap, K2 extends keyof PromptMap[K1]>(
	origin: K1,
	condition: K2,
): Prompt {
